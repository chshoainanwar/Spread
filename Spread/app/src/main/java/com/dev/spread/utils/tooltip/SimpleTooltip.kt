...
package com.dev.spread.utils.tooltip
...
import com.dev.amintopup.base.screenSizeHeight
import com.dev.amintopup.base.screenSizeWidth
import com.dev.spread.R
import com.example.ikon.utils.tooltip.ArrowDrawable
import com.example.ikon.utils.tooltip.OverlayView
import com.example.ikon.utils.tooltip.SimpleTooltipUtils
...
class SimpleTooltip private constructor(builder: Builder) : PopupWindow.OnDismissListener {
    ...
    var mArrowLayoutListener = object : ViewTreeObserver.OnGlobalLayoutListener {
        override fun onGlobalLayout() {
            ...
            if (mShowArrow) {
                ...
                val contentViewRect = SimpleTooltipUtils.calculateRectOnScreen(
                    mContentLayout
                        ?: return
                )
    ...
    fun show() {
        ...
        mRootView?.post {
            if (mRootView?.isShown == true)
                mPopupWindow?.showAtLocation(mRootView, Gravity.NO_GRAVITY, mRootView?.width
                        ?: screenSizeWidth, mRootView?.height
                        ?: screenSizeHeight)
            else
    ...
    private fun createOverlay() {
        ...
        if (mOverlayMatchParent)
            ...
            mOverlay?.layoutParams = ViewGroup.LayoutParams(mRootView?.width
                    ?: screenSizeWidth, mRootView?.height ?: screenSizeHeight)
    ...
    private fun configContentView() {
        ...
        if (mShowArrow) {
            mArrowView = mContext?.let { AppCompatImageView(it) }
