package com.dev.amintopup.base.animtation
...
class CircularWheelView : ConstraintLayout {
    ...
    private fun getAttributedValues(attrs: AttributeSet) {
        ...
        textSize = a.getDimensionPixelSize(R.styleable.CircularWheelView_wheel_item_text_size, 20)
            .toFloat()
        normalColor = a.getColor(R.styleable.CircularWheelView_wheel_item_text_color, Color.GRAY)
        selectionColor =
            a.getColor(R.styleable.CircularWheelView_wheel_item_selected_text_color, Color.WHITE)
        wheelBackgroundColor =
            a.getColor(R.styleable.CircularWheelView_wheel_background_color, Color.BLACK)
        a.recycle()
    }
    ...
    private fun configureWheel() {
        wheelLayout.viewTreeObserver.addOnGlobalLayoutListener(object :
            ...
            override fun onGlobalLayout() {
                if (wheelLayout.measuredHeight > 1) {
                    ...
                    runTimeWidth =
                        (max(wheelLayout.measuredHeight, wheelLayout.width) * 0.9).toInt()
                    with(param) {
                        height = runTimeWidth
                        width = runTimeWidth
                        if (this@CircularWheelView.measuredWidth > width / 2) {
                            if (viewType == LEFT)
                                marginEnd =
                                    (this@CircularWheelView.measuredWidth - width * 0.4).toInt()
                            else
                                marginStart =
                                    (this@CircularWheelView.measuredWidth - width * 0.4).toInt()
                        }
    ...
    private fun configureWheelElements() {
        maxElementsCount =
            min(((runTimeWidth / getBiggestTextSize().first) * 4).toInt(), itemList.size)
        if (maxElementsCount % 2 != 0)
            maxElementsCount--
        ROTAION_ANGLE_OFFSET =
            if (viewType == LEFT) 360.0f / maxElementsCount.toFloat() else (360.0f / maxElementsCount.toFloat()) * -1f
    }
        ...
        (0 until maxElementsCount / 2).forEach {
            val textView = TextView(context).apply {
                ...
                layoutParams = ConstraintLayout.LayoutParams(
                    ...
                    circleRadius =
                        (((dummyView.measuredHeight / 2) - (dummyView.measuredHeight / 2) * 0.2f).toInt())
    ...
    private fun getBiggestTextSize(): Pair<Float, Float> {
        ...
        return Pair(
            sqrt((size.first * size.first + size.second * size.second).toFloat()) + DEFAULT_IN_BETWEEN_SPACE,
            size.second.toFloat()
        )
    }

    private fun getTextViewSize(
        context: Context,
        text: CharSequence,
        textSize: Float,
        deviceWidth: Int,
    ...
    private fun performUpScrollingTask(gap: Int) {
        var start =
            roundToCorrectIndex(lastIdlePosition + (maxElementsCount / 2) - gap, maxElementsCount)
        val item = getChildOfWheelByIndex(start)
        var startValueIndex =
            getNextPositionFor(itemList.indexOf(item.text.toString()), itemList.size)
        (0 until gap).forEach {
    ...
    private fun onScrollFinished(animationDuration: Long = 100) {
        ...
        if (mPieRotation % ROTAION_ANGLE_OFFSET != 0.0f) {
            val choice: String
            val condition =
                abs(mPieRotation % ROTAION_ANGLE_OFFSET) < abs(ROTAION_ANGLE_OFFSET / 2f)
        ...
        ObjectAnimator.ofFloat(wheelLayout, "rotation", oldRotation, mPieRotation).apply {
            ...
            addListener(object : Animator.AnimatorListener {
                override fun onAnimationEnd(animation: Animator) {
                    ...
                    wheelItemSelectionListener?.onItemSelected(
                        itemList.indexOf(
                            getChildOfWheelByIndex(currentPosition).text.toString()
                        )
                    )
